...
const LINES: &str = include_str!("../../problem_inputs_2018/day_19.txt");
...
pub fn solution() -> ((usize, Duration), (usize, Duration)) {
    ...
    (
        solve(instruction_pointer, &instructions, false),
        solve(instruction_pointer, &instructions, true),
    )
}
...
fn solve(
    ...
    let mut cpu = CPU::new(&instructions, instruction_pointer, part_2 as usize);
    loop {
        if let Some(result) = cpu.step() {
...
impl Instruction {
    fn from_str(s: &str) -> Self {
        ...
        let opcode = match parts.next().unwrap() {
            ...
            "banr" => OpCode::Ban(InputType::Reg, InputType::Reg),
            "bani" => OpCode::Ban(InputType::Reg, InputType::Imm),
            "borr" => OpCode::Bor(InputType::Reg, InputType::Reg),
            "bori" => OpCode::Bor(InputType::Reg, InputType::Imm),
...
impl<'a> CPU<'a> {
    ...
    fn step(&mut self) -> Option<usize> {
        ...
        self.instruction_pointer = self.registers[self.ip_bound] + 1;
        None
    }
}
